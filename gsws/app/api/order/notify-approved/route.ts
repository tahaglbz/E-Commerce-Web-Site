...
import { OrderApprovedEmailTemplate } from '@/lib/email-templates';
import { Order, OrderItem } from '@/app/types';

const resend = new Resend(process.env.RESEND_API_KEY);

interface NotifyApprovedRequestBody {
  orderId: number;
}

export async function POST(request: NextRequest) {
  try {
    const body: NotifyApprovedRequestBody = await request.json();
    ...
    const emailHtml = OrderApprovedEmailTemplate({
    ...
    const response = await resend.emails.send({
      ...
      subject: `🎉 Siparışınız Onaylandı #${order.id}`,
      html: emailHtml,
    });
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'Sipariş onaylandı bildirimi maili başarıyla gönderildi!',
    ...
    console.error('Order Approved Notification API Error:', error);

    return NextResponse.json(
      {
        error: 'Sipariş onaylandı bildirimi gönderme sırasında bir hata oluştu.',
