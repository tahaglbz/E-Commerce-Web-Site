...
import { OrderPlacedEmailTemplate } from '@/lib/email-templates';
import { Order, OrderItem } from '@/app/types';

const resend = new Resend(process.env.RESEND_API_KEY);

interface NotifyPlacedRequestBody {
  orderId: number;
}

export async function POST(request: NextRequest) {
  try {
    const body: NotifyPlacedRequestBody = await request.json();
    ...
    const response = await resend.emails.send({
      ...
      subject: `✅ Siparışınız Alındı #${order.id}`,
      html: emailHtml,
    });
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'Sipariş alındı bildirimi maili başarıyla gönderildi!',
    ...
    console.error('Order Placed Notification API Error:', error);

    return NextResponse.json(
      {
        error: 'Sipariş alındı bildirimi gönderme sırasında bir hata oluştu.',
